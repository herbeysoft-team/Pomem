...
import {
  ...
  updateDepartment,
  deleteDepartment,
} from "../../Context/features/departmentSlice";
...
const Department = () => {
  ...
  const { departments, loading, error } = useSelector((state) => ({
    ...state.department,
  }));
  const { dept_name } = formValue;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (dept_name) {
      dispatch(createDepartment({ formValue, navigate, toast }));
      setTimeout(() => {
        setOpen(false);
        dispatch(getDepartments());
      }, 500);
  ...
  const handleEdit = (id) => {
    ...
    if (id) {
      ...
      const deptDetails = departments.find((data) => data.id == id);
      setFormValue(deptDetails);
      setDeptId(id);
    }
  };
  //function called to handle edit
        ...
        >
          ...
          <DialogActions>
            ...
            <Button
              color="success"
              onClick={() => {
                dispatch(deleteDepartment({ deptId, toast }));
                setTimeout(() => {
                  setOpenDialog(false);
                  dispatch(getDepartments());
                }, 500);
